...
public class VelocityUtils {
	...
	public static String getContentByTemplate(Object model, String templateFilePathAndName){
		try {
			...
			StringWriter writer = new StringWriter();//实例化一个StringWriter
	...
	public static void staticByTemplate(Object model, String templateFilePathAndName, String targetFilePathAndName) {
		try {
			Velocity.init(p);
			Template template = Velocity.getTemplate(templateFilePathAndName);//拿到模版
			
			VelocityContext context = new VelocityContext();//创建一个VelocityContext
			context.put("model", model);//放入数据对象
			...
			template.merge(context, writer);//模版合并数据
			writer.close();
		} catch (Exception e) {
			e.printStackTrace();
		}
